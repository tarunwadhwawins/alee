...
function DragDropPage() {
    ...
    const exportForm = (data) => {
        ...
        for (let i = 0; i < JSON.parse(data).length; i++) {
            const fieldsData = { fieldId: 0, fieldName: JSON.parse(data)[i].label.blocks[0].text, fieldDataTypeId: (commonFunctions.getGlobalCodeDetails(globalCode, "DataType", JSON.parse(data)[i].element)).globalCodeId, fieldOrder: i + 1, isRequired: JSON.parse(data)[0].required, categoryName: null }
